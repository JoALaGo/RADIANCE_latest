...
function findInstanceInStorage(parent_index, instance_id) {
    ...
    alertify.error("No instance found with id: "+instance_id+" and parent: "+parent_index);
    return null;
}
...
function findInstanceInProfile(parent_index, instance_id, profile) {
    ...
    alertify.error("findInstanceinProfile: No instance found with id: "+instance_id+" and parent: "+parent_index);
    return null;
}
...
function downloadJSON(type, id) {
    if (type == "schema") {
        ...
        let place = findInstancePlaceInStorage(type, id);
        let text = JSON.stringify(rebuildJSON(db[type + "_Array"][place].profile_cache));
        console.log(text);
        var element = document.createElement('a');
        element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(text));
        element.setAttribute('download', db[type + "_Array"][place].element_name);
...
function saveRawEdit() {
    ...
    if (type == 'schema') {
        //CHANGE: add a function that handles the changes in the root schema to improve the support for managing the changes in the parent schema and schema updates 
        manageParentSchemaChanges(db[type + "_Array"][storage_place].element_content, JSON.parse(document.getElementById("raw_text_field").value), element_to_edit.element_id);
...
var old_dissected_schema=[];
var new_dissected_schema=[];
function manageParentSchemaChanges(old_schema, new_schema) {
    //old_schema is the previous version, a raw text json
    //new_schema is the newest version of the schema and it is already in json
    let type = "schema";
    let storage_place = findInstancePlaceInStorage(type, element_to_edit.element_id);
    //determine if the schema is a consumption guide
    let is_guide = db[type + "_Array"][storage_place].is_guide = document.getElementById('the_modified_schema_is_guide').checked;
    let change;
    (JSON.stringify(old_schema) === JSON.stringify(new_schema) ? console.log("No changes to the parent schema found") : change = true);

    if (change) {
        //check what changed by dissecting it to arrays and comparing the two arrays
        dissectToObjects(old_schema, old_dissected_schema);
        dissectToObjects(new_schema, new_dissected_schema);
    }
