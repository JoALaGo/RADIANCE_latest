...
var algorithm_results_copy=[];
var timing_diagram_scores = [];
var general_scatter_plot = null;
var algorithm_setup ={};
...
function teachConfiguration(current_level) {
    current_level == null ? current_level = findConfigurationVariables() : current_level;
    ...
    if (checkDuplicatedVariables()) {
        ...
        console.log("New length after slashing duplicates: (original "+this.available_variables.length+"/"+result.length+" w/o duplicates)");
...
function treatDuplicatedVariables(array) {
    console.log("treatDuplicatedVariables: "+array);
...
function dissectToObjects2(obj, target) {
    let from = 'dissectToObjects: ';
    let temporal_element = JSON.parse(JSON.stringify(obj));
    dismemberObject(temporal_element, '');
    let result = backtrackParents(target);
    console.log(from + " hierarchy established");
        ...
        if (typeof obj === 'object') {
            ...
            for (let key of keys) {
                ...
                if (typeof value === 'object' && !Array.isArray(value)) {
                    target.push([key.replace(/-/g, "_"), JSON.parse(JSON.stringify(value).replace(/-/g, "_")), 'object', '', []]);
                    delete obj[key];
                    dismemberObject(value);
                } else if (Array.isArray(value)) {
                    for (let i = 0; i < value.length; i++) {
                        ...
                        if (typeof item === 'object') {
                            target.push([`${key}_${i}`.replace(/-/g, "_"), JSON.parse(JSON.stringify(item).replace(/-/g, "_")), 'array', '', []]);
                            dismemberObject(item);
                        }
                    }
                }
            }
    ...
    function backtrackParents(array) {
        for (let i = array.length - 1; i >= 0; i--) {
            let [target_name, target_content] = array[i];
            array[i][3] = 'root';
            for (let x = i - 1; x >= 0; x--) {
                let [source_name, source_content] = array[x];
                if (isParent(source_content, target_name)) {
                    array[i][3] = x;
                    break;
                }
            }
...
function saveRawPropertyValue(parent_index, input_id, property_name, is_variable_subset, is_instance, type, instance_id, is_custom, subset_name, custom_value) {
    ...
    if (!is_instance) {
        console.log("saveRawPropertyValue: It is not an instance");
        let input_value;
        if (is_custom) {
            console.log("saveRawPropertyValue: It is a customized field");
            input_value = custom_value;
        } else {
            console.log("saveRawPropertyValue: It is not a customized field");
            input_value = document.getElementById(property_name + '_value_field_' + input_id).value;
        }
        if(input_value==null){
            alert("The value could not be obtained by querying the ui dom field for its value (does the ui element exist?)");
            return;
        }
        ...
        alertify.success('Succesfully saved the new value '+input_value+' for ' + property_name.split('_').join(' '));
        ...
        console.log("saveRawPropertyValue: It is an instance");
        let input_value;
        if (is_custom) {
console.log("saveRawPropertyValue: It has a customized input.");
            //if something changes, we have to alert the user to run the algorithm again
        ...
        for (let x in available_instances) {
            ...
            if (available_instances[x].inner_id === instance_id) {
                ...
                if (is_variable_subset) {
                    ...
                    if(!found_Subset){
                        alert('A proper place to store the value in could not be found');
                    }
                ...
                console.log('%c saveRawPropertyValue: No instances associated to the instance ID were found. Retrying by making it NOT an instance.', 'color:red');
                //saveRawPropertyValue(parent_index, input_id, property_name, is_variable_subset, false, type, instance_id, is_custom, subset_name, custom_value);
...
function createInstanceFromProfileElement(library_id, dom_target, parent_index, self_index, is_parent, is_instance, parent_id, parent_instance_id) {
    ...
    //we call the timing diagram draft.
...
function runBuilderAlgorithm() {
    ...
    if (this.db.builder_algorithm !== null && this.db.builder_algorithm !== '') {
        try {
            algorithm_results_copy = eval(this.db.builder_algorithm);
        } catch (error) {
...
function sequencerRulesReasoner(array) {
    ...
    function populateResults() {
        ...
        try{
            var results_instances = array[findPlaceByParentName('Results', array)][4];
        }catch(e){
            console.error("The profile is not BBCP-compliant. Interrupting the sequence diagram generation after attempting to fetch messages (which are results in the context of the BBCP).");
            return;
        }
    ...
    function populateDependencies() {
        ...
        //lets fetch the results
        try{
            var dependencies_instances = array[findPlaceByParentName('Operational', array)][4];
        }catch(e){
            console.log("This profile does not seem to be BBCP-compliant. Skipping dependencies.");
            return;
        }
    ...
    function populateTriggers() {
        ...
        //lets fetch the results
        try{
            var triggers_instances = array[findPlaceByParentName('Triggers', array)][4];
        }catch (e){
            console.log("This profile does not seem to be BBCP-compliant. Aborting triggers.");
            return;
        }
        ...
        for (let x in triggers_instances) {
            ...
            //lets add the origin
            for (let y in available_operations) {
                console.log("Operation ID: "+triggers_instances[x].inner_variables[0].variables.operation_ID+" Inner ID: "+available_operations[y].inner_id+" Structure: "+JSON.stringify(triggers_instances[x].inner_variables[0]));
...
function getProfileOperationalPathways(profile, sequencer, step) {
    ...
    const getTriggers = () => profile[findPlaceByParentName('Triggers', profile)][4];
    const getResults = () => profile[findPlaceByParentName('Results', profile)][4];
    const getOperationName = (operation_id) => {
        const operationsIndex = findPlaceByParentName('Operations', profile);
        return findInstanceInStorage(operationsIndex, operation_id).Name;
    };

    const getParameterName = (parameter_id) => {
        const parametersIndex = findPlaceByParentName('Parameters', profile);
        return findInstanceInStorage(parametersIndex, parameter_id);
    };
    ...
    if (step === 0) {
        ...
        for (const trigger of triggers) {
            const { trigger_Type, trigger_value, operation_id } = trigger.inner_variables[0].variables;
            if (trigger_Type === 'state' && trigger_value === 'run') {
                const operationName = getOperationName(operation_id);
                const pseudonym = operationName.split(' ').join('');
                new_step.description += `\n@${step + 1}\n${pseudonym} is On`;
                new_step.by_state.push(operation_id);
                new_step.description += `\n@${step + 2}\n${pseudonym} is Off`;
            }
        }

        getOperationsCascade(new_step.by_state, profile, new_step.by_operations, new Set());

        for (const operation_id of new_step.by_operations) {
            const operationName = getOperationName(operation_id);
            const pseudonym = operationName.split(' ').join('');
            new_step.description += `\n@${step + 1}\n${pseudonym} is On`;
        }
        ...
        for (const result of results) {
            const { source_operation, parameter } = result.inner_variables[0].variables;
            if (new_step.by_state.includes(source_operation) || new_step.by_operations.includes(source_operation)) {
                new_step.results_generated.push(parameter);
            }
        }

    } else {
        const previous_step = sequencer[step - 1];
        const triggers = getTriggers();

        for (const result of previous_step.results_generated) {
            for (const trigger of triggers) {
                const { trigger_Type, trigger_value, operation_id } = trigger.inner_variables[0].variables;
                if (trigger_Type === 'parameter' && trigger_value === result) {
                    const operationName = getOperationName(operation_id);
                    const pseudonym = operationName.split(' ').join('');
                    new_step.by_parameter.push(operation_id);
                    new_step.description += `\n@${step + 1}\n${pseudonym} is On`;
                    new_step.description += `\n@${step + 2}\n${pseudonym} is Off`;
                }
            }
        ...
        getOperationsCascade(new_step.by_parameter, profile, new_step.by_operations, new Set());
        ...
        for (const result of results) {
            const { source_operation, parameter } = result.inner_variables[0].variables;
            if (new_step.by_parameter.includes(source_operation) || new_step.by_operations.includes(source_operation)) {
                new_step.results_generated.push(parameter);
            }
        }
        const prevOperations = previous_step.by_operations || [];
        for (const operation_id of prevOperations) {
            const operationName = getOperationName(operation_id);
            const pseudonym = operationName.split(' ').join('');
            new_step.description += `\n@${step + 1}\n${pseudonym} is Off`;
        }
        if (new_step.by_operations.length === 0 && new_step.by_parameter.length === 0) return;
    }

    sequencer[step] = new_step;
    getProfileOperationalPathways(profile, sequencer, step + 1);
}



function getOperationsCascade(startOperations, profile, rootCascade, visited = new Set()) {
    let cascade = [];
    ...
    for (let x in startOperations) {
        for (let y in triggers_index) {
            ...
            let operation_to_trigger = trigger_data.operation_id;
            let triggered_by = trigger_data.trigger_value;
            let trigger_Type = trigger_data.trigger_Type;

            console.log(`%cChecking: does ${startOperations[x]} trigger ${operation_to_trigger}?`, 'color:blue');

            if (startOperations[x] === triggered_by && trigger_Type === 'operation') {
                if (!visited.has(operation_to_trigger)) {
                    console.log(`%cYES - adding ${operation_to_trigger}`, 'color:green');
                    visited.add(operation_to_trigger);
                    rootCascade.push(operation_to_trigger);
                    cascade.push(operation_to_trigger);
                } else {
                    console.log(`%cSKIP - already visited ${operation_to_trigger}`, 'color:orange');
                }
            } else {
    ...
    if (cascade.length > 0) {
        getOperationsCascade(cascade, profile, rootCascade, visited);
    }
}
    ...
    for (let x in startOperations) {
        for (let y in triggers_index) {
            ...
            if (startOperations[x] == trigger_target && trigger_Type == 'operation') {
                ...
                console.log("rootCascade so far: " + JSON.stringify(rootCascade));
...
//----------This section is dedicated to rebuilding the JSON from the flattened version----
function rebuildJSON(flatArray) {
    // If flatArray is a string, try to parse it.
    if (typeof flatArray === "string") {
      try {
        flatArray = JSON.parse(flatArray);
      } catch (err) {
    ...
    // Convert the flat array into nodes.
    const nodes = flatArray.map((item, index) => {
      const [name, defaults, type, parent, innerData] = item;
      return {
        index,
        name,
        defaults,
    ...
    // Build a lookup map and assign children based on parent references.
    const nodeMap = {};
    nodes.forEach(node => {
    ...
    nodes.forEach(node => {
      if (node.parent === "root") {
        ...
        if (p) {
          ...
          console.warn(`Parent ${node.parent} not found for node ${node.name}`);
        }
      }
    });
  
    // Helper: Rename keys to match your schema (e.g., "general_traits" -> "general-traits")
    function renameKey(key) {
      if (key === "general_traits") return "general-traits";
      if (key === "sub_categories") return "sub-categories";
      return key;
    }
    ...
    function processNode(node) {
      if (node.type === "object") {
        // Start with the node defaults.
        const result = { ...node.defaults };
        // Process children and add as properties.
        node.children.forEach(child => {
          result[renameKey(child.name)] = processNode(child);
        });
        return result;
      } else if (node.type === "array") {
        // Create a template from defaults and children.
        let template = {};
        if (node.defaults && typeof node.defaults === "object" && !Array.isArray(node.defaults)) {
          template = { ...node.defaults };
        }
        node.children.forEach(child => {
          template[renameKey(child.name)] = processNode(child);
        });
        // Build the final array.
        const arr = [];
        if (node.innerData && Array.isArray(node.innerData) && node.innerData.length > 0) {
          node.innerData.forEach(instance => {
            const instanceObj = {};
            if (instance.inner_variables && Array.isArray(instance.inner_variables)) {
              instance.inner_variables.forEach(iv => {
                if (iv.name === "root") {
                  // Merge the properties of "root" directly into the instance.
                  Object.assign(instanceObj, iv.variables);
                } else {
                  instanceObj[renameKey(iv.name)] = iv.variables;
                }
              });
            }
            // Merge template with instance (instance values override template defaults).
            arr.push({ ...template, ...instanceObj });
          });
        } else {
          arr.push(template);
        }
    ...
    // Assemble the final output by merging all root-level nodes.
    const output = {};
    roots.forEach(root => {
      // Add the processed node directly at top level under its (renamed) name.
      output[renameKey(root.name)] = processNode(root);
    });
